...
- **Client Shortlist**: Dedicated page (`/projects/:id/client-shortlist`) displaying accepted experts (pipeline status: interested/shortlisted/accepted) with read-only view of expert profiles, employment history, vetting Q&A, and availability slots. Includes professional PDF export functionality via `GET /api/projects/:id/client-shortlist-export` using PDFKit with multi-page layout for client presentations. Authorization restricts access to Admin, PM, and assigned RAs.
