...
- **Project-Level Consultations Tab**: The project detail page includes a dedicated "Consultations" tab (between RA Sourcing and Activity) that displays all call records for that specific project. Features summary stats (Scheduled, Completed, CUs Used), a status filter dropdown, Schedule Call modal for creating new consultations, and Mark Completed modal for completing calls with actual duration. RAs can view but cannot schedule or complete calls. The tab leverages the existing CallRecord entity filtered by projectId.
