...
- **Project-Level Consultations Tab**: The project detail page includes a dedicated "Consultations" tab (between RA Sourcing and Activity) that displays all call records for that specific project. Features summary stats (Scheduled, Completed, CUs Used), a status filter dropdown, Schedule Call modal for creating new consultations, Mark Completed modal for completing calls with actual duration, and Edit Call modal for updating duration/CU with manual override capability. CU calculation uses formula: `ceil(durationMinutes / 15) * 0.25` (1 CU = 60 min, billed in 0.25 CU / 15 min increments). Manual CU override allowed with 0.25 increment validation. RAs can view but cannot schedule, edit, complete, or cancel calls. The tab leverages the existing CallRecord entity filtered by projectId.
